...
async function main() {
    ...
    let myHandler = new class MyHandler extends Handler {
        ...
        async handleIncoming(event) {
            ...
            console.log(JSON.stringify(this.eventParser.parseIncomingEvent(event)));
            return ['1', '2'];
        }
    }();

    // Init provider
    // !!! Provider use wrappers from ./contracts directory
    ...
    let event = await myProvider.listenQueries(filters, oracle);
