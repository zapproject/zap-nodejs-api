...
const ZapProvider = require('./ZapProvider.js');

class ZapIPFSProvider extends ZapProvider {
    constructor(eth, network, keypair) {
        super(eth, network);
    ...
    listen(callback) {
        super.listen((err, event) => {
            ...
            if ( args.length != 2 ) {
                callback(new Error("Got invalid arguments from a subscription event"));
                return;
            }
...
module.exports = ZapIPFSProvider;
