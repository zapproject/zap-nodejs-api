...
class SynapseProvider {
    ...
    checkForRegister(configFile, group, wei_rate, callback) {
        ...
        if (fs.existsSync(configFile)) {
            ...
            this.keypair = crypto.createECDH('secp224r1');
    ...
    initSubscription(blocknumber, data) {
        ...
        let uuid_buf = Buffer.concat([cipher.update(cipher_text,'hex'), cipher.final()]);
	const uuid = uuid_buf.toString('base64');
    console.log("ctext",cipher_text);
    console.log("uuid buf",uuid_buf);
...
const provider = new SynapseProvider("danny_01", 1);
//provider.on('ready', () => {})
